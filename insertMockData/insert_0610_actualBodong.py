...
for i in range(1, 8):
    ...
    for j in range(points):
        ...
        if j > points - 300:  # 最后300个点为异常数据
            # 逐渐超出正常范围并保持异常
            amp += np.random.uniform(0.05, 0.1)  # 逐渐增加，直到超出范围
            rate += np.random.uniform(0.05, 0.1)
            temp += np.random.uniform(0.1, 0.2)
            vol -= np.random.uniform(0.05, 0.1)  # 低于最小值
            f_alarm = 1  # 标记为异常
        else:
            f_alarm = 0  # 标记为正常

        # 生成21位唯一ID
        unique_id = str(uuid.uuid4().int)[:21]  # 生成21位唯一ID
